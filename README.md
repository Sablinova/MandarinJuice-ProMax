...
- **Extreme Bruteforce Optimization:** To bruteforce an unknown Account ID, the application must run a massive loop testing all 4.2 billion possible 32-bit integers (`uint.MaxValue`). The original `MandarinJuice` used a slow looping `Splitmix64` hash generation within `TryParsedUserId` that, when executed 4.2 billion times, pegged the CPU at 100% for over 1 hour to run against Steam64 IDs, severely starving VPS resources. This fork heavily optimizes it by completely unrolling the 16 hashing rounds and byte-extraction loops with early-exits. This eliminates the prolonged extreme CPU overhead, dropping the time required to calculate all 4.2 billion combinations and bruteforce Resident Evil 9 User IDs to just ~58 seconds on target hardware. This prevents server resource exhaustion and allows the tool to run efficiently within Discord bot interaction limits.
- **Multithreading Loop Optimization:** Due to testing 4.2 billion combinations, any overhead within the inner loop is extremely expensive. The original CLI tool calculated progress UI updates inside the main bruteforce `Parallel.For` loop (using modulo operations and thread-blocking `Interlocked.Add` calls millions of times per second). This fork completely rips out the progress tracking from the hot inner loop, shifting it to only report when a thread partition fully completes. This removes massive synchronization locks and branching overhead across 4.2 billion iterations.
